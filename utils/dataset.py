...
from stat import calc_tokens
from stat import stat_para_nums
from stat import stat_sen_nums
...
if __name__ == '__main__':
    ...
    for i in range(len(wiki_doc_complex)):
        wiki_avg_token += calc_tokens(wiki_doc_complex[i])
        newsela_avg_token += calc_tokens(newsela_docs[i][0])
    ...
    for i, j in zip(complex_col, simple_col):
        if 500 > calc_tokens(i) > 300:
            ...
            words_a = calc_tokens(i)
            words_b = calc_tokens(j)
            avg_words_a += words_a
            avg_words_b += words_b
    ...
    for doc, content in zip(docs, contents):
        ...
        if calc_tokens(v0) <= 1000:
            ...
            all_word_a += calc_tokens(v0)
            all_word_b += calc_tokens(v1 + v2 + v3 + v4) / 4
